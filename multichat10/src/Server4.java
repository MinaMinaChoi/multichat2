...
public class Server4 {
    ...
    private HashMap<String, UserInfo> rooms = new HashMap<String, UserInfo>();
...
/*                //1.현재같은 방이 존재하는지 확인
                ...
                if (RoomCh) {//방을 만들 수 있을때
                    ...
                    users.put(userid, user_socket);
                    rooms.put(Message, this);

                    System.out.println(Message + "/" + userid + "/" + user_socket);
              ...
              //  UserInfo userInfo = rooms.get(Message);

                Iterator iterator = rooms.get(Message).users.keySet().iterator();

                while (iterator.hasNext()) {
                    iterator.next();
                    rooms.get(Message).send_Message("Chatting|" + Message + "|" + userid + "|" + image + "|" + msg);
                    System.out.println(Message + " | " + userid + " : " + msg);

                }

                if (user_socket != rooms.get(Message).users.get(userid)) { //나빼고 방사람들에게....아니지...다보내야하는군....
             ...
             /*   for (int i = 0; i < roomList.size(); i++) { //방찾기
               ...
               // UserInfo userInfo = rooms.get(Message);
                ...
                rooms.get(Message).users.put(userid, user_socket);

                rooms.get(Message).send_Message("Chatting|" + Message + "|알림| |" + nickName + "입장했습니다");
              //  userInfo.BroadCast("Chatting|" + Message + "|알림| |" + nickName + "입장했습니다", user_socket);
