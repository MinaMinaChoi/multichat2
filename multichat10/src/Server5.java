            ...
            while (true) {
                try {
                    ...
                    if (left_packet > 0) {
                        ...
                        System.out.println("인풋스트림에서 읽어온길이 : " + buf.length);
                    }
                    ...
                    //     protocol.setPacket2(total_len, buf); //total크기의 바이트배열에다가 지금까지 받은 buf(바이트배열)을 복사하고,

                    while (recv_len < total_len) { //받아야할 길이보다 덜 받았으면....
                        if (is.available() > 0) {
                            if (total_len - recv_cnt - buf.length < 0) { //마지막으로 읽어들일때는 남은 바이트 크기만큼만 버퍼크기지정.
                                buf = new byte[total_len - recv_len];
                            } else {
                    ...
                    for (int i = 0; i < roomList.size(); i++) {
                        ...
                        if (r.Room_name.equals(roomid)) {
                           ...
                           // String msgtype = Protocol.Message;
                            ...
                            protocol = new Protocol(msg.getBytes().length + 113);
                            protocol.setProtocolType(String.valueOf(Protocol.PT_CHAT_MSG));
                            protocol.setTotalLen(String.valueOf(msg.trim().getBytes().length + 113));
              ...
              //      }
                ...
                case Protocol.PT_CHAT_IMG :
                    ...
                    //if (msgtype.equals(Protocol.IMG)) { // 이미지
                        ...
                        byte[] data = protocol.getImg(Integer.parseInt(totalLen1) - 113);
                        ...
                        long curr = System.currentTimeMillis();
                        byte[] temp = new byte[Integer.parseInt(totalLen1) - 113];

                        System.arraycopy(protocol.getPacket(), 113, temp, 0, Integer.parseInt(totalLen1) - 113);
        ...
        public void sendToRoomuser(byte[] packet) {
            for (int i = 0; i < roomuserlist.size(); i++) {
                ...
                for (int j = 0; j < userList.size(); j++) { //여기서 두번.
                    ...
                    // System.out.println("서버유저리스트 " + userInfo.userid);
                    if (userid.equals(userInfo.userid)) {
                        ...
                        //   System.out.println( userid+"에게 " + packet.toString() +" 보냄");
                    }
