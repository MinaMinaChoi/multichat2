...
public class Server2 {
    ...
    private List<UserThread> userList = new ArrayList<UserThread>();
            ...
            public void run() { //스레드에서 처리할 일을 기재
                ...
                while (true) {
                    try {
                        ...
                        UserThread userInfo = new UserThread(socket);
    ...
    class UserThread extends Thread {
        ...
        UserThread(Socket socket) { //생성자 메소드
...
/*
        public void broadcast_room(String hostid) {
            ...
            try {
                ...
                String sql = "SELECT * FROM roomusertable WHERE hostroomid = "+hostid;

                ResultSet rs = stmt.executeQuery(sql);

                while (rs.next()) {
                  //  userthread.send_message("");
                }
...
*/
        ...
        private void BroadCast(String str) { //전체사용자에게 메세지를 보내는부분
            for (int i = 0; i < userList.size(); i++) {
                UserThread user = userList.get(i);
...
//            System.out.println("프로토콜 : "+protocol);
            ...
            if (protocol.equals("CreateRoom")) { //postgres 디비에서 방을 가져오는 것으로 변경...
                //1.현재같은 방이 존재하는지 확인
                for (int i = 0; i < roomList.size(); i++) {
                ...
                String msg = st.nextToken(); //채팅내용, Chatting/방이름/채팅내용

                for (int i = 0; i < roomList.size(); i++) { //방찾기
                    RoomInfo r = roomList.get(i);
                    if (r.Room_name.equals(Message)) {//해당방을 찾았을 때
                        r.BroadCast_Room("Chatting/" + nickName + "/" + msg); //내이름으로 다른 사람들에게 메시지 전달
...
/*
                try {
                    ...
                    while (rs.next()) {
                        String roomid = String.valueOf(rs.getInt("hostid"));

                        if (roomid == Message) {
                            
                            InsertUser(roomid, nickName);
                            
                            
                            //roomusertable에 추가하기!!
                            //해당방 사람들에게, 새로운 사용자가 입장했음을 알리고,
...
*/
                    ...
                    if (r.Room_name.equals(Message)) {
                       ...
                       /* for (int j = 0; j < r.OutingList.size(); j++) { //해당방의 강제퇴장목록을 확인
                        ...
                        if (JoinCh) { //방에 입장할 수 있는 상태이면... 해당방에 강제퇴장적이 없으면
                            //새로운 사용자를 알린다
                            r.BroadCast_Room("Chatting/알림/" + nickName + "입장했습니다");

                            //사용자 추가
                            r.Add_User(this);

                            try {
                                InsertUser(Message, nickName);
                            } catch (SQLException e) {
                                e.printStackTrace();
                            }
                ...
                for (int i = 0; i < roomList.size(); i++) {
                    ...
                    if (r.Room_name.equals(Message)) { //해당방에서 강제퇴장ID지움
                        for (int k = 0; k < r.RoomUserList.size(); k++) {
                            UserThread userInfo = (UserThread) r.RoomUserList.get(k);
    ...
    public void InsertUser(String hostid, String userid) throws SQLException {
        try {
            conn = DriverManager.getConnection("jdbc:postgres://13.124.77.49:5432/postgres", "postgres", "alsdkek123!");

            stmt = conn.createStatement();

            String sql = "INSERT INTO roomusertable (hostroomid, userid) VALUES ("+hostid+", "+userid+")";
    ...
    class RoomInfo {
        ...
        RoomInfo(String str, UserThread u) {
            this.Room_name = str;
        ...
        public void BroadCast_Room(String str) {//현재 방에 속한 유저들을 하나씩 꺼내서, 채팅내용을 보내준다
            for (int i = 0; i < RoomUserList.size(); i++) {
                UserThread u = (UserThread) RoomUserList.get(i);
                u.send_Message(str);
        ...
        private void Add_User(UserThread u) {
            this.RoomUserList.add(u);
